...
int main()
{
    ...
    vector<string> samples = {
        "k10dcf1d147c4197fba20c86f3d59b777"};
    ...
    for (string &sample : samples)
    {
        ...
        while ((isSuccessful = scrollInstance.next(res)))
        {
            ...
            {
                uint64_t ts = hit["_source"]["TimeStamp"].asInt64();
                while (!cq.empty() && cq.front()["_source"]["TimeStamp"].asInt64() < ts)
                {
                    Event event = parser.parse(cq.front());
                    cq.pop();
                }
                if (hit["_source"]["Event"].asString() == "ProcessEnd")
                    cq.push(hit);
                else
